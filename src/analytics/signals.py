...
def process_table_metric(sender, instance, created, **kwargs):
    ...
    try:
        ...
        if file_extension.lower() == ".csv":
            ...
            raise ValueError(
                "Invalid file type. Only CSV or Excel files are supported."
            )

        if df.empty:
            print(f"⚠️ Warning: The file {file_path} is empty.")
        ...
        # ✅ Convert DataFrame to JSON without NaN issues
        table_data = {
            "columns": list(df.columns),
            "data": json.loads(df.to_json(orient="records")),  # Ensures valid JSON
        }

        # ✅ Store in TableMetric (ensuring no NaN errors)
