document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", async function (e) {
        ...
        if (driveLink) {
            ...
            await confirmUpload(null, title, driveLink);
            return;
        }
        ...
        if (file.size > MAX_SIZE_BYTES) {
            ...
            setTimeout(() => {
                uploadLargeFile(file, title);
            }, 500);
        } else {
            console.log("📦 Small file, using direct POST upload");
            await uploadSmallFile(file, title);
        }
    });

    async function uploadSmallFile(file, title) {
        ...
        xhr.onload = async function () {
            if (xhr.status === 204 || xhr.status === 201) {
                await confirmUpload(file_key, title, null);
            } else {
                alert("❌ Upload failed.");
                reset();
            }
    ...
    async function uploadLargeFile(file, title) {
    ...
    async function confirmUpload(file_key, title, drive_link) {
